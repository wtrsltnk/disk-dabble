...
class Statement {
 public:
  Statement(sqlite3* db, const char* query, int len)
      : stmt_(new_sqlite3_stmt(db, query, len), sqlite3_stmt_deleter) {}
 ...
 private:
  ...
  sqlite3_stmt* new_sqlite3_stmt(sqlite3* db, const char* query, int len) {
    if (len == -1)
    {
        len = static_cast<int>(strlen(query));
    }
    sqlite3_stmt* p = nullptr;
    verify(sqlite3_prepare(db, query, len, &p,
                           nullptr));
    return p;
  }
...
class Sqlite {
 public:
  ...
  Statement<void> prepare(const char* query, int len) const {
    return Statement<void>(db_, query, len);
  }

  template <typename... Types>
  Statement<Types...> prepare(const char* query, int len) const {
    return Statement<Types...>(db_, query, len);
  }

  template <typename... Args>
  void execute(const char* query, int len, const Args&... args) {
    prepare<void>(query, len).execute(args...);
  }
